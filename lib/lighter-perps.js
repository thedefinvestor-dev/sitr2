...
/** Funding rates across venues; "lighter" rows are ours.
 *  NOTE: `rate` is the NEXT 8-HOUR rate (verified: settled hourly decimal × 8
 *  matches the live rate on stable markets like SGOV). Mark prices come from
 *  orderBookDetails (funding-rates rows carry no price). */
async function fetchLighterRates(bases = []) {
  const [j, markets] = await Promise.all([
    lighterGet('/api/v1/funding-rates', {}, 'Lighter funding rates'),
    fetchLighterMarkets().catch(() => ({})),
  ]);
  ...
  for (const row of rows) {
    ...
    out.push({
      venue: 'lighter',
      symbol,
      fundingRateHourly: rate / 8,
      fundingRate8h: rate,
      fundingRateInterval: rate,
      fundingIntervalHours: 8,
      markPx: markets[symbol]?.markPx ?? null,
    });
  }
  return out;
}
...
async function fetchLighterState(authority) {
  ...
  for (const row of acc.positions || []) {
    ...
    positions.push({
      ...
      // total_funding_paid_out accumulates positively as RECEIVED funding
      // (verified against the fundings feed: shorts on positive-rate markets
      // like VVV receive — longs pay). Positive = income, negative = cost.
      fundingSinceOpen: lighterNum(row.total_funding_paid_out, 0),
      leverage: null,
    });
  }
...
/**
 * Public per-account funding-history estimate: the market-wide `fundings` feed
 * carries every hourly settlement for a market (rate per unit, no auth). Given
 * the account's current position size we can reconstruct what the account paid/
 * received each hour. Accurate while the position size is unchanged (the common
 * delta-neutral hold); flagged `estimated` so the UI can label it.
 */
async function fetchLighterFundingEstimated(authority, days = 30) {
  const empty = { venue: 'lighter', wallet: authority, days, payments: [], totalFunding: 0 };
  const wallet = String(authority || '').trim();
  if (!isEthAddress(wallet)) return empty;
  const windowStart = Date.now() - days * 86400000;
  ...
  let state;
  try {
    state = await fetchLighterState(wallet);
  } catch (e) {
    return { ...empty, error: e.message };
  }
  if (!state?.positions?.length) return empty;

  const markets = await fetchLighterMarkets().catch(() => ({}));
  const payments = [];
  for (const pos of state.positions) {
    const meta = markets[pos.symbol];
    if (!meta) continue;
    const startSec = Math.floor(windowStart / 1000);
    const endSec = Math.floor(Date.now() / 1000);
    let j;
    try {
      j = await lighterGet('/api/v1/fundings', {
        market_id: meta.marketId,
        resolution: '1h',
        start_timestamp: startSec,
        end_timestamp: endSec,
        count_back: 750,
      }, `Lighter fundings ${pos.symbol}`);
    } catch (e) {
      continue;
    }
    const rows = Array.isArray(j?.fundings) ? j.fundings : [];
    // Newest first: the bounded walk below must consume the position's real
    // cumulative funding from the MOST RECENT settlements — otherwise the
    // estimate lands entirely on the oldest hours and pair-window stats (e.g.
    // "since open") show 0 for the Lighter leg.
    rows.sort((a, b) => Number(b.timestamp) - Number(a.timestamp));
    // The market-wide feed assumes the CURRENT size existed for every past hour,
    // which wildly overstates funding for recent positions (e.g. a position open
    // for hours would show weeks of payments). Bound it with the position's real
    // cumulative funding (total_funding_paid_out): keep newest payments until the
    // accumulated |cashflow| matches the real total — the same "since-open"
    // behavior funding history gives on other exchanges.
    const realTotal = Math.abs(Number(pos.fundingSinceOpen) || 0);
    let acc = 0;
    for (const row of rows) {
      const time = Number(row.timestamp) * 1000;
      if (!time || time < windowStart) continue;
      // `value` = per-unit payment in USD (matches value/mark = rate fraction);
      // `rate` is a PERCENT-style number (e.g. 0.0038 = 0.38%), NOT a fraction —
      // using it as a fraction inflated payments ~5-100x.
      const perUnit = lighterNum(row.value, NaN);
      if (!Number.isFinite(perUnit) || !perUnit) continue;
      // `direction` names the side that PAYS (verified: short VVV with
      // direction 'long' rows accumulated +5.81 received — positive rate →
      // longs pay shorts). Payment: if we ARE the paying side → cost (-),
      // else income (+).
      const dir = String(row.direction || '').toLowerCase() === 'short' ? 'short' : 'long';
      const wePayingSide = pos.side === dir;
      const mag = perUnit * Math.abs(pos.size);
      acc += mag;
      payments.push({
        venue: 'lighter',
        time,
        symbol: pos.symbol,
        usdc: wePayingSide ? -mag : mag,
        size: pos.size,
        fundingRate: (perUnit / (markets[pos.symbol]?.markPx || NaN)),
        intervalHours: LIGHTER_FUNDING_INTERVAL_HOURS,
        fundingEstimated: true,
      });
      if (realTotal > 0 && acc >= realTotal * 0.98) break; // reached position's real since-open total
    }
  }
  payments.sort((a, b) => b.time - a.time);
  return {
