...
async function fetchLighterFundingEstimated(authority, days = 30) {
  ...
  for (const pos of state.positions) {
    ...
    for (const row of rows) {
      ...
      // Positive value = the OPPOSITE of `direction` pays (verified: short 600
      // VVV paid 2.65 total while rows had value>0, direction 'long' —
      // i.e. `direction` names the RECEIVING side).
