 ...
 *  Mark prices come from orderBookDetails (funding-rates rows carry no price). */
...
async function fetchLighterFundingEstimated(authority, days = 30) {
  ...
  for (const pos of state.positions) {
    ...
    for (const row of rows) {
      ...
      const rate = lighterNum(row.rate, NaN);
      if (!Number.isFinite(rate)) continue;
      // positionFunding `change` = rate × position_size (size in base units);
      // sign follows the position side (long pays positive rate).
      payments.push({
        venue: 'lighter',
        time,
        symbol: pos.symbol,
