...
function resolveHedgeTrackedSignedSize(hedge, trackedLegOrSize = null) {
  if (hedgeIsPartial(hedge)) {
    ...
    // A partial hedge never claims the full live leg: when the live leg exists it
    // must still be >= the partial (the residual slice lives inside the leg).
    if (Number.isFinite(liveTracked) && liveTracked !== 0) {
      const absLive = Math.abs(liveTracked);
      const partial = hedgePartialSize(hedge);
      if (partial > 0 && partial <= absLive + 1e-9) {
        return Math.sign(liveTracked) * partial;
      }
    }
  ...
  for (const hedge of nextHedges) {
    ...
    if (hedge.status === 'open') {
      ...
      if (liveTrackedLeg) {
        ...
        hedge.supersededByLiveCross = false;
        // A partial (residual) hedge must offset only the residual slice of the
        // live tracked leg — never the whole leg (which is already partly hedged
        // by an existing live cross on the same symbol+venue).
        const hedgeTrackedLeg = hedgeIsPartial(hedge)
          ? (buildPartialTrackedLeg(liveTrackedLeg, hedgePartialSize(hedge)) || liveTrackedLeg)
          : liveTrackedLeg;
        hedge.trackedLastSnapshot = {
