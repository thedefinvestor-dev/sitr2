...
function buildPairedAnalysis({
  ...
  /** Prefer best size match across venue pairs — do not let a tiny opposite leg steal the hedge.
   *  Allow up to 50% mismatch so intentional partial hedges (e.g. Ext 30 / Phoenix 45) still pair
   *  with a size_mismatch alert instead of appearing fully unhedged. */
  const OPEN_PAIR_MAX_SIZE_MISMATCH_PCT = 50;
  ...
  for (const base of [...allBases].sort()) {
    ...
    for (const c of candidates) {
      ...
      if (c.mismatch > OPEN_PAIR_MAX_SIZE_MISMATCH_PCT) continue;
      if (c.spec.hlNado) {
