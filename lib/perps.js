...
const {
  ...
  fetchLighterState,
  fetchLighterFunding,
  fetchLighterFundingEstimated,
} = require('./lighter-perps');
...
/** Transient upstream errors (rate limits) — retried server-side; showing them
 *  as dashboard warnings is noise since the next refresh usually succeeds. */
function isTransientVenueError(msg) {
  const s = String(msg || '').toLowerCase();
  return s.includes('rate_limited') || s.includes('rate limited') || s.includes('too many requests') || s.includes('429');
}
...
function combineErrors(...items) {
  return items
    ...
    .filter(Boolean)
    .filter(msg => !isTransientVenueError(msg))
    .join('; ') || null;
}
...
async function fetchPerpsEquitySnapshot(wallets) {
  ...
  const lighterWallet = String(wallets.lighter || wallets.lighterWallet || '').trim();
  const lighterEnabled = /^0x[0-9a-fA-F]{40}$/.test(lighterWallet);
  const perpl = wallets.perpl || null;
  const perplEnabled = Boolean(perpl?.apiKey && perpl?.secret);
  const [hl, nado, grvt, extended, phoenix, lighter, perplState] = await Promise.all([
    ...
    lighterEnabled
      ? withTimeout(fetchLighterState(lighterWallet), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Lighter state').then((s) => ({
        venue: 'lighter',
        configured: true,
      ...
      : Promise.resolve({ venue: 'lighter', configured: false, accountValue: 0 }),
    perplEnabled
  ...
  const states = [hl, nado, grvt, extended, phoenix, lighter, perplState];
  ...
  const lighterAccountValue = lighterEnabled ? (lighter.accountValue ?? 0) : 0;
  const perplAccountValue = perplEnabled ? perplState.accountValue : 0;
  const total = hlAccountValue + nadoAccountValue + grvtAccountValue + extendedAccountValue + phoenixAccountValue + lighterAccountValue + perplAccountValue;
  ...
  return {
    ...
    summary: {
      ...
      lighterConfigured: lighterEnabled,
...
function fundingForVenueInWindow(venue, base, maps) {
  ...
  if (venue === 'lighter') return maps.lighter[base] || 0;
  return 0;
}
...
function buildDailyFundingSeries({
  ...
  addFunding(lighterPayments, 'lighter');
  addFees(hlFills);
  addFees(nadoMatches);
  addFees(grvtFills);
...
function venuesForPairPerformance(pair) {
  ...
  if (/lighter/.test(pt)) out.add('lighter');
  return [...out];
}
...
function filterDailySeriesInputsByVenues(dailySeriesInputs, venues) {
  ...
  return {
    ...
    lighterPayments: pick(dailySeriesInputs?.lighterPayments, 'lighter'),
...
function buildPairDailyPerformanceSeries(dailySeriesInputs, symbol, perfDays, endMs = Date.now(), venues = null) {
  ...
  return buildDailyFundingSeries({
    ...
    lighterPayments: inputs.lighterPayments || [],
...
function buildPairedAnalysis({
  ...
  const lighter = lighterState || { positions: [] };
  ...
  const lighterByBase = Object.fromEntries(lighter.positions.map(p => [toBaseSymbol(p.symbol), p]));
  ...
  const fundingLighter = lighterFunding ? sumByBase(lighterFunding.payments, 'usdc') : {};
  const fundingMaps = {
    hl: fundingHl,
    nado: fundingNado,
    grvt: fundingGrvt,
  ...
  const paymentSources = {
    ...
    lighter: lighterFunding?.payments || [],
  };
  const fillSources = {
  ...
  const dailySeriesInputs = {
    ...
    lighterPayments: lighterFunding?.payments || [],
  ...
  function getFundingSinceOpen(venue, leg, base) {
    ...
    if (venue === 'lighter') return leg.fundingSinceOpen ?? fundingLighter[base] ?? 0;
    return 0;
  }
  ...
  const pairSpecs = [
    ...
    { venues: ['hyperliquid', 'lighter'], pairType: 'hl_lighter', pairLabel: 'HL + Lighter', spreadKey: 'spreadHlLighter8h' },
    { venues: ['nado', 'lighter'], pairType: 'nado_lighter', pairLabel: 'Nado + Lighter', spreadKey: 'spreadNadoLighter8h' },
    { venues: ['grvt', 'lighter'], pairType: 'grvt_lighter', pairLabel: 'GRVT + Lighter', spreadKey: 'spreadGrvtLighter8h' },
    { venues: ['extended', 'lighter'], pairType: 'extended_lighter', pairLabel: 'Extended + Lighter', spreadKey: 'spreadExtendedLighter8h' },
    { venues: ['phoenix', 'lighter'], pairType: 'phoenix_lighter', pairLabel: 'Phoenix + Lighter', spreadKey: 'spreadPhoenixLighter8h' },
    { venues: ['perpl', 'lighter'], pairType: 'perpl_lighter', pairLabel: 'Perpl + Lighter', spreadKey: 'spreadPerplLighter8h' },
  ];
  ...
  const legSpecs = [
    ...
    { venue: 'lighter', map: lighterByBase, funding: (p, b) => p.fundingSinceOpen ?? fundingLighter[b] ?? 0, fees: feesPerpl },
  ];
...
function buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase = {}, phoenixRateByBase = {}, perplRateByBase = {}, lighterRateByBase = {}) {
  const spreadRows = [];
  for (const base of bases) {
    ...
    const li = lighterRateByBase[base];
    if (!hl && !na && !gv && !ex && !vr && !ph && !pp && !li) continue;
    ...
    const lighter8h = li?.fundingRate8h ?? null;
    ...
    spreadRows.push({
      ...
      lighter8h,
      lighterMarkPx: li?.markPx ?? null,
      lighterHourly: li?.fundingRateHourly ?? (lighter8h != null ? lighter8h / 8 : null),
      variational8h,
      ...
      spreadHlLighter8h: hl8h != null && lighter8h != null ? hl8h - lighter8h : null,
      spreadNadoLighter8h: na8h != null && lighter8h != null ? na8h - lighter8h : null,
      spreadGrvtLighter8h: grvt8h != null && lighter8h != null ? grvt8h - lighter8h : null,
      spreadExtendedLighter8h: extended8h != null && lighter8h != null ? extended8h - lighter8h : null,
      spreadPhoenixLighter8h: phoenix8h != null && lighter8h != null ? phoenix8h - lighter8h : null,
      spreadPerplLighter8h: perpl8h != null && lighter8h != null ? perpl8h - lighter8h : null,
...
async function fetchPerpsLiveRates(opts = {}) {
  ...
  const [hlRates, nadoRates, grvtRates, extendedRates, variationalRates, phoenixRates, lighterRates, perplRates] = await Promise.all([
    ...
    withTimeout(fetchLighterRates([...bases]), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Lighter rates').catch(() => []),
  ...
  const lighterRateByBase = Object.fromEntries(lighterRates.map(r => [r.symbol, r]));
  const perplRateByBase = Object.fromEntries(perplRates.map(r => [r.symbol, r]));

  return {
    fetchedAt: Date.now(),
    rateSpread: buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase, phoenixRateByBase, perplRateByBase, lighterRateByBase),
  };
}
...
async function fetchPerpsDashboard(wallets, opts = {}) {
  ...
  const [
    ...
    perplEnabled ? withTimeout(fetchPerplCapitalFlows(perpl), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Perpl capital flows').catch(e => ({
      venue: 'perpl', wallet: perpl?.apiKey || null, payments: [], netDeposits: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'perpl', payments: [], netDeposits: 0 }),
    lighterEnabled ? withTimeout(fetchLighterState(lighterWallet), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Lighter state').catch(e => ({
      venue: 'lighter', wallet: lighterWallet, configured: true, exists: false, accountValue: 0, positions: [], error: errorMessage(e),
    })) : Promise.resolve({
      venue: 'lighter', wallet: lighterWallet || null, configured: false, exists: false, accountValue: 0, positions: [],
    }),
    lighterEnabled ? withTimeout(fetchLighterFunding(lighterWallet, fillHistoryDays), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Lighter funding').then(async (funding) => {
      // Authenticated history needs a Lighter read-only auth token; without one,
      // fall back to the PUBLIC market-wide fundings feed × current position size
      // (estimated) so daily funding + recent payments still show for Lighter.
      if (funding?.authRequired) {
        const est = await withTimeout(
          fetchLighterFundingEstimated(lighterWallet, fillHistoryDays),
          PERPS_OPTIONAL_FETCH_TIMEOUT_MS,
          'Lighter funding est',
        ).catch(() => null);
        if (est && est.payments.length) return est;
      }
      return funding;
    }).catch(e => ({
      venue: 'lighter', wallet: lighterWallet, days: fillHistoryDays, payments: [], totalFunding: 0, error: errorMessage(e),
    })) : Promise.resolve({ venue: 'lighter', wallet: lighterWallet || null, days: fillHistoryDays, payments: [], totalFunding: 0 }),
  ]);
  ...
  const [grvtRates, extendedRates, variationalRates, phoenixRates, lighterRates, perplRates] = await Promise.all([
    grvtEnabled
    ...
    withTimeout(fetchLighterRates([
      ...(lighterState?.positions || []).map(p => p.symbol),
      'BTC', 'ETH', 'SOL',
    ]), PERPS_OPTIONAL_FETCH_TIMEOUT_MS, 'Lighter rates').catch(() => []),
  ...
  const bases = new Set([
    ...
    ...Object.keys(lighterRateByBase),
    'BTC', 'ETH', 'SOL',
  ]);
  const spreadRows = buildRateSpreadRows(bases, hlRateBySymbol, nadoRateByBase, grvtRateByBase, extendedRateByBase, variationalRateByBase, phoenixRateByBase, perplRateByBase, lighterRateByBase);
  ...
  const lighterEquity = lighterEnabled ? (lighterState.accountValue ?? 0) : 0;
  const equityNow = {
    ...
    perpl: perplEquity,
    lighter: lighterEquity,
    total: hlState.accountValue + (nadoState.accountValue ?? 0) + grvtEquity + extendedEquity + phoenixEquity + perplEquity + lighterEquity,
    adjustedTotal: hlState.accountValue + (nadoState.accountValue ?? 0) + grvtEquity + extendedEquity + phoenixEquity + perplEquity + lighterEquity - capitalFlows.combinedNetDeposits,
  };
  const equitySeries = buildEquitySeries({
    ...
    lighterAccountValue: lighterEquity,
    fetchedAt,
    snapshots: [],
    backfill: [],
  });
  ...
  const dashboard = {
    ...
    lighter: {
      ...
      fills: { fills: [], totalFees: 0, totalRealized: 0 },
      configured: lighterEnabled,
    },
    extended: {
      state: extendedState,
    ...
    summary: {
      ...
      lighterFundingTotal: lighterFunding.totalFunding,
      netFundingTotal: hlFunding.totalFunding + nadoFunding.totalFunding + grvtFunding.totalFunding + extendedFundingWindow.totalFunding + phoenixFunding.totalFunding + perplFunding.totalFunding + lighterFunding.totalFunding,
      ...
      lighterPositionCount: (lighterState?.positions || []).length,
      hlAccountValue: hlState.accountValue,
      nadoAccountValue: nadoState.accountValue ?? 0,
      ...
      lighterConfigured: lighterEnabled,
      // Funding history needs a Lighter read-only auth token — treat the graceful
      // degradation as expected, not a connection error (positions/state still work).
      lighterError: lighterEnabled
        ? combineErrors(lighterState, { error: lighterFunding?.authRequired ? null : lighterFunding?.error })
        : null,
      lighterAccountValue: lighterEquity,
      perplConfigured: perplEnabled,
      perplError: combineErrors(perplState, perplFunding, perplFills, perplCapitalFlows),
      perplAccountValue: perplEquity,
      lighterEquitySnapshot: lighterEquity,
      ...
      netFundingTotalAllAccounts: hlFunding.totalFunding + nadoFunding.totalFunding + grvtFunding.totalFunding + extendedFundingWindow.totalFunding + phoenixFunding.totalFunding + perplFunding.totalFunding + lighterFunding.totalFunding,
  ...
  if (capitalFlows && Number.isFinite(capitalFlows.combinedNetDeposits)) {
    const total = equityBase.equityNow?.total
      ?? ((equityBase.summary?.hlAccountValue || 0)
        ...
        + (equityBase.summary?.lighterAccountValue || 0)
