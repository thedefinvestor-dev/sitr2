...
const PHOENIX_FUNDING_OVERVIEW_CACHE_MS = 60 * 1000;
const PHOENIX_MARK_CACHE_MS = 30 * 1000;
// Phoenix public REST rate-limits bursts (HTTP 429 { error: "rate_limited" }).
// Pace every request through one process-level token bucket and back off on 429.
const PHOENIX_REQS_PER_SEC = 5;
const PHOENIX_BURST = 5;
const PHOENIX_RATE_LIMIT_RETRIES = 2;
const PHOENIX_RATE_LIMIT_BACKOFF_MS = 1000;

/** Minimal token bucket: resolves when the caller may send one request. */
function createTokenBucket({ ratePerSec, burst }) {
  const capacity = Math.max(1, Number(burst) || 1);
  const refillPerMs = Math.max(0.001, Number(ratePerSec) || 1) / 1000;
  let tokens = capacity;
  let last = Date.now();
  const waiters = [];
  let draining = false;
  ...
  return function acquire() {
    return new Promise((resolve) => {
      ...
      if (!draining) drain().catch(() => {});
    });
  };
}

function phoenixIsRateLimited(data, text) {
  const error = String(data?.error || text || '').toLowerCase();
  return error.includes('rate_limit') || error.includes('rate limit') || error.includes('too many requests');
}
...
function phoenixActiveTriggerPx(triggers, tickSize, entryPriceTicks, entryPriceUsd) {
  ...
  // The markets API `tickSize` is NOT a reliable price divisor (e.g. VVV tickSize
  // 10 but price = ticks / 1000; MON tickSize 1 but price = ticks / 1,000,000).
  // The position's own entry is authoritative and shares the same tick system as
  // the trigger ticks (entryPriceUsd = entryPriceTicks / divisor), so derive the
  // divisor from it and fall back to the market tickSize only when unavailable.
  const entryTicks = Number(entryPriceTicks);
  const entryUsd = Number(entryPriceUsd);
  const derivedTickSize = (Number.isFinite(entryTicks) && entryTicks !== 0
    && Number.isFinite(entryUsd) && entryUsd > 0)
    ? entryTicks / entryUsd
    : null;
  const effectiveTickSize = (derivedTickSize != null && derivedTickSize > 0) ? derivedTickSize : tickSize;
  for (const row of rows) {
    ...
    let px = phoenixTicksToUsd(ticks, effectiveTickSize);
    if (px == null) {
      // Last-resort direct derivation from the position's own entry ticks/USD.
...
function createPhoenixApi({ fetchWithTimeout, withTimeout, errorMessage, toBaseSymbol, timeoutMs }) {
  ...
  const phoenixLimiter = createTokenBucket({
    ratePerSec: PHOENIX_REQS_PER_SEC,
    burst: PHOENIX_BURST,
  });
  let _marketsCache = null;
  let _marketsCacheAt = 0;
  let _fundingOverview = null; // { at, data }
  let _fundingOverviewInflight = null;
  let _marksCache = null; // { at, bySymbol }
  const _marksInflight = {};
  ...
  async function phoenixGet(path, params = {}, label = 'Phoenix') {
    ...
    let last = null;
    for (let attempt = 0; attempt <= PHOENIX_RATE_LIMIT_RETRIES; attempt++) {
      await phoenixLimiter();
      ...
      if (res.status !== 429 && !phoenixIsRateLimited(data, text)) break;
      const retryAfterSec = Number(res.headers.get('retry-after'));
      const backoffMs = Number.isFinite(retryAfterSec) && retryAfterSec > 0
        ? retryAfterSec * 1000
        : PHOENIX_RATE_LIMIT_BACKOFF_MS;
  ...
  async function fetchPhoenixRates(bases = []) {
    let overview = _fundingOverview?.data;
    if (!overview || Date.now() - _fundingOverview.at > PHOENIX_FUNDING_OVERVIEW_CACHE_MS) {
      if (!_fundingOverviewInflight) {
        _fundingOverviewInflight = phoenixGet('/v1/funding/overview', {}, 'Phoenix funding overview')
          .then((res) => {
            _fundingOverview = { at: Date.now(), data: res.ok ? res.data : null };
            return _fundingOverview;
          })
          .finally(() => { _fundingOverviewInflight = null; });
      }
      const cached = await _fundingOverviewInflight;
  ...
  /** Live mark price with a short TTL + in-flight dedupe (biggest burst source). */
  async function fetchPhoenixMark(symbol) {
    const cachedPx = _marksCache?.bySymbol?.[symbol];
    if (cachedPx != null && Date.now() - _marksCache.at <= PHOENIX_MARK_CACHE_MS) return cachedPx;
    if (!_marksInflight[symbol]) {
      _marksInflight[symbol] = (async () => {
        try {
          ...
          const value = Number.isFinite(px) && px > 0 ? px : null;
          if (value != null) {
            if (!_marksCache || Date.now() - _marksCache.at > PHOENIX_MARK_CACHE_MS) {
              _marksCache = { at: Date.now(), bySymbol: {} };
            }
            _marksCache.bySymbol[symbol] = value;
          }
          return value;
        } catch (_) {
          return null; // keep overview fallback
        } finally {
          delete _marksInflight[symbol];
        }
      })();
    }
    return _marksInflight[symbol];
  }
  ...
  async function fetchPhoenixState(authority) {
    ...
    await Promise.all([...new Set(openSymbols)].slice(0, 24).map(async (symbol) => {
      const px = await fetchPhoenixMark(symbol);
      if (px != null) markBySymbol[symbol] = px;
    }));
...
module.exports = {
  ...
  PHOENIX_REQS_PER_SEC,
  PHOENIX_BURST,
  PHOENIX_RATE_LIMIT_RETRIES,
  isSolanaAddress,
  isBlockedPhoenixAuthority,
  isUsablePhoenixWallet,
  phoenixParseTs,
