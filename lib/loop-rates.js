...
const AAVE_V4_GQL = 'https://api.v4.aave.com/graphql';
...
/** Aave V4 (Hub & Spoke): one UserPosition per spoke. The v4 GraphQL API
 *  (api.v4.aave.com) exposes userSupplies/userBorrows with USD exchange values
 *  and per-reserve APYs — same canonical loop shape as V3, protocol 'AaveV4'. */
let _aaveV4ChainIds = null;
async function fetchAaveV4ChainIds() {
  if (_aaveV4ChainIds) return _aaveV4ChainIds;
  const data = await gql(AAVE_V4_GQL, 'query { chains(request: { query: { filter: ALL } }) { name chainId } }', {}, {
    headers: { Origin: 'https://app.aave.com', Referer: 'https://app.aave.com/' },
    timeout: 20000,
  });
  _aaveV4ChainIds = (data.chains || []).map(c => c.chainId).filter(Number.isFinite);
  return _aaveV4ChainIds;
}

function groupAaveV4Positions(wallet, supplies, borrows, userPositions = []) {
  const healthBySpoke = new Map();
  for (const up of userPositions || []) {
    const key = `${up?.spoke?.chain?.chainId ?? '?'}:${up?.spoke?.id || up?.spoke?.name || '?'}`;
    const hf = up?.healthFactor?.current;
    if (hf != null) healthBySpoke.set(key, num(hf, null));
  }
  const bySpoke = new Map();
  const ensure = (reserve) => {
    const chain = reserve?.chain || {};
    const spoke = reserve?.spoke || {};
    const key = `${chain.chainId ?? '?'}:${spoke.id || spoke.name || '?'}`;
    if (!bySpoke.has(key)) {
      bySpoke.set(key, {
        id: `aave-v4:${loopWalletKey(wallet)}:${key}`,
        protocol: 'AaveV4',
        source: 'aave-v4-api',
        confidence: 'high',
        wallet,
        ...
        marketName: spoke.name || 'Aave V4 spoke',
        marketAddress: spoke.address || null,
        _spokeHealth: healthBySpoke.get(key) ?? null,
        supplied: [],
        borrowed: [],
        totalSupplied: 0,
        totalBorrowed: 0,
    ...
    return bySpoke.get(key);
  };

  for (const item of supplies || []) {
    const usd = num(item?.balance?.exchange?.value);
    if (usd <= 0.01) continue;
    const apy = percent(item?.reserve?.summary?.supplyApy?.normalized);
    const row = ensure(item.reserve);
    row.supplied.push({
      symbol: item?.reserve?.asset?.underlying?.info?.symbol || 'Asset',
      value: usd,
      amount: num(item?.balance?.amount?.value),
      apy,
      address: item?.reserve?.asset?.underlying?.address,
      isCollateral: Boolean(item?.isCollateral),
    });
    row.totalSupplied += usd;
    row.suppliedYieldUsd += usd * apy;
  ...
  for (const item of borrows || []) {
    const usd = num(item?.debt?.exchange?.value);
    if (usd <= 0.01) continue;
    const apy = percent(item?.reserve?.summary?.borrowApy?.normalized);
    const row = ensure(item.reserve);
    row.borrowed.push({
      symbol: item?.reserve?.asset?.underlying?.info?.symbol || 'Debt',
      value: usd,
      amount: num(item?.debt?.amount?.value),
      apy,
      address: item?.reserve?.asset?.underlying?.address,
  ...
  return [...bySpoke.values()]
    ...
    .map(p => {
      ...
      p.netApy = p.totalBorrowed > 0.01 ? netApy(p) : p.supplyApy;
      p.lendingOnly = p.totalBorrowed <= 0.01 && p.totalSupplied > 0.01;
      p.health = p._spokeHealth;
      delete p._spokeHealth;
      return p;
    });
}

async function fetchAaveV4Wallet(wallet) {
  const chainIds = await fetchAaveV4ChainIds();
  if (!chainIds.length) return [];
  const query = `query AaveV4Loops($chains:[ChainId!]!, $user:EvmAddress!) {
    userSupplies(request:{query:{userChains:{user:$user,chainIds:$chains}},orderBy:{amount:DESC}}) {
      reserve { chain { chainId name } spoke { id name address } asset { underlying { address info { symbol } } } summary { supplyApy { normalized } } }
      balance { amount { value } exchange { value } }
      isCollateral
    }
    userBorrows(request:{query:{userChains:{user:$user,chainIds:$chains}},orderBy:{amount:DESC}}) {
      reserve { chain { chainId name } spoke { id name address } asset { underlying { address info { symbol } } } summary { borrowApy { normalized } } }
      debt { amount { value } exchange { value } }
    }
    userPositions(request:{user:$user,filter:{chainIds:$chains}}) {
      spoke { id name chain { chainId } }
      healthFactor { current }
    }
  }`;
  const data = await gql(AAVE_V4_GQL, query, { chains: chainIds, user: wallet }, {
    headers: { Origin: 'https://app.aave.com', Referer: 'https://app.aave.com/' },
    timeout: 35000,
  });
  return groupAaveV4Positions(wallet, data.userSupplies, data.userBorrows, data.userPositions);
}
...
async function fetchLoopRates({ wallets }) {
  ...
  if (!evmWallets.length && !solanaWallets.length) {
    return {
      ...
      coverage: { aave: AAVE_MARKETS, aaveV4: { source: 'api.v4.aave.com/graphql', model: 'hub-and-spoke' }, morpho: MORPHO_CHAINS, morphoMidnight: MORPHO_MIDNIGHT_CHAINS, fluid: FLUID_CHAINS, kamino: [], jupiterLend: [] },
    };
  }
  ...
  if (evmWallets.length) {
    ...
    const aaveV4Settled = await Promise.allSettled(evmWallets.map(fetchAaveV4Wallet));
    aaveV4Positions = aaveV4Settled.flatMap((r, i) => {
      if (r.status === 'fulfilled') return r.value;
      errors.push({ provider: 'aave-v4', wallet: evmWallets[i], message: r.reason?.message || 'Aave V4 fetch failed' });
      return [];
    });
  ...
  let preEnrichPositions = [...aavePositions, ...aaveV4Positions, ...morphoPositions, ...fluid.positions, ...sparkSavings.positions, ...sparkLend.positions, ...solana.positions]
  ...
  return {
    ...
    coverage: {
      aave: AAVE_MARKETS,
      aaveV4: { source: 'api.v4.aave.com/graphql', model: 'hub-and-spoke' },
